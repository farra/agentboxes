...
Today, agentboxes provides orchestrator shells (schmux, gastown, openclaw) that assume agents (claude, aider, codex) are pre-installed on the host system. This works for:
...
There's no way to say "I want schmux with claude and aider and python 3.12" in a single declarative configuration that works for both local development and container images.
...
User has claude, aider already installed on their machine.
...
CANCELLED: Skipping aider for initial implementation. Focus on claude and codex first.
...
Export devShells.claude, devShells.aider, devShells.codex for standalone agent use.
...
Skip aider for initial implementation. Focus on claude and codex first.
