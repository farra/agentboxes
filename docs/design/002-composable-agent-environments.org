...
Define a agentbox.toml configuration pattern that allows users to declaratively specify which orchestrator, agents, and language runtimes they need. This enables self-contained development environments and OCI images with specific agent combinations baked in.
...
1. Single agentbox.toml file defines the complete environment
...
No agentbox.toml needed. This continues to work.
...
nix develop
# Reads agentbox.toml, composes shell with orchestrator + agents + runtimes
...
# Build OCI image from agentbox.toml
nix build .#image
docker load < result
...
# agentbox.toml is committed to the repo
cat agentbox.toml
# [orchestrator]
# name = "gastown"
# [agents]
...
These are just pre-made agentbox.toml combinations.

* Design

** agentbox.toml Schema
...
This design adds:
- =lib/mkProjectShell.nix= - Compose shell from agentbox.toml
- =lib/mkProjectImage.nix= - Compose OCI image from agentbox.toml
- =agents/<name>/default.nix= - Agent packages
- =templates/project/= - Template with flake.nix that reads agentbox.toml
...
│   ├── mkProjectShell.nix        # NEW: agentbox.toml → devShell
│   └── mkProjectImage.nix        # NEW: agentbox.toml → OCI image
├── agents/
│   ├── claude/
...
├── templates/
│   └── project/
│       ├── flake.nix             # Reads agentbox.toml
│       └── agentbox.toml             # Example config
...
Core function that reads agentbox.toml and composes a devShell with:
- Substrate
...
- flake.nix that imports agentboxes and reads agentbox.toml
- Example agentbox.toml
...
** TODO [AB-002-09] Document agentbox.toml format                          :p2:
:PROPERTIES:
:EFFORT: S
:END:
...
Rely on flake.lock for reproducibility. No explicit version pinning in agentbox.toml for now.
