...
** TODO [AB-003-01] Create orchestrators/ralph/default.nix              :p1:
:PROPERTIES:
:EFFORT: M
:END:
...
** TODO [AB-003-02] Update flake.nix with ralph orchestrator            :p1:
:PROPERTIES:
:EFFORT: S
:END:
...
** TODO [AB-003-03] Update mkProjectShell for ralph auto-include        :p1:
:PROPERTIES:
:EFFORT: S
:END:
...
** TODO [AB-003-04] Test ralph shell end-to-end                         :p2:
:PROPERTIES:
:EFFORT: M
:END:
...
** TODO [AB-003-05] Document ralph usage                                :p2:
:PROPERTIES:
:EFFORT: S
:END:
...
** OPEN Should we vendor ralph or use a flake input?
...
Cons:
- Manual updates (git submodule update)
- Larger repo size

Recommendation: Keep vendoring for now. Consider flake input if ralph gets its own nix packaging upstream.

** OPEN How to handle ralph's install.sh global install?
...
** OPEN Should ralph-setup be included?
