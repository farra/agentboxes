...
** agentbox.toml Integration

When =orchestrator.name = "ralph"= in agentbox.toml:
...
# Option 2: Use agentbox.toml
mkdir my-project && cd my-project
nix flake init -t github:farra/agentboxes#project
# Edit agentbox.toml: orchestrator.name = "ralph"
nix develop
ralph-enable
ralph --monitor
#+end_example
...
When orchestrator is "ralph", auto-include claude agent even if not specified in agentbox.toml.
