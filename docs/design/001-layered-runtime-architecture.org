...
** OPEN How to handle substrate versioning

If substrate changes, how do existing users update? Pin via flake.lock? Auto-update?

** OPEN deps.toml vs flake.nix for projects

Should we prioritize deps.toml (simpler) or flake.nix composition (more flexible)? Or both?
