...
│          Source: project's flake.nix or agentbox.toml      │
├─────────────────────────────────────────────────────────┤
...
*** Option B: agentbox.toml (simpler, AI-friendly)

#+begin_src toml
# project agentbox.toml
[base]
...
agentbox.toml is the "per-instance configuration" layer - simpler than flake.nix for common cases.
...
5. Each workspace can have its own =flake.nix= or =agentbox.toml=
...
- agentbox.toml for simple project configuration
...
Enable agentbox.toml → devShell for project-specific configuration.
...
** DECIDED agentbox.toml vs flake.nix for projects
:PROPERTIES:
:DECIDED: [2026-02-01]
:END:

Support both. agentbox.toml for simpler user configuration of their specific project. flake.nix always available for power users who need full flexibility.
