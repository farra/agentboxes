...
- TOML configuration format (=deps.toml= in cautomaton-develops, =agentbox.toml= in agentboxes)
...
agentboxes should:
- Mirror the configuration pattern from cautomaton-develops (using =agentbox.toml=)
...
** TODO [AB-004-01] Align agentbox.toml format with cautomaton-develops    :p1:
:PROPERTIES:
:EFFORT: M
:END:

Update agentboxes =mkProjectShell.nix= to use a similar =agentbox.toml= structure as
cautomaton-develops' =deps.toml=:
